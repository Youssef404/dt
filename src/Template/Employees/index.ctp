            ...
            <table class="table table-striped table-bordered" id="datatable-button">
                ...
                <?php foreach ($employees as $employee): ?>
                    <tr>
                        ...
                        <td class="actions">
                            <?= $this->Html->link($this->Html->tag('i',"",['class'=>'fa fa-folder']).__('Afficher'), ['action' => 'view', $employee->id],['class'=>'btn btn-primary btn-xs ','escape'=>false]) ?>
                            <?= $this->Html->link($this->Html->tag('i',"",['class'=>'fa fa-pencil']).__('Modifier'), ['action' => 'edit', $employee->id],['class'=>'btn btn-info btn-xs ','escape'=>false]) ?>
                            <?= $this->Form->postLink($this->Html->tag('i',"",['class'=>'fa fa-trash-o'])._('Supprimer'), ['action' => 'delete', $employee->id],['class'=>'btn btn-danger btn-xs ','escape'=>false,'confirm' => __('Êtes vous sûr de vouloir supprimer {0} {1} ?', $employee->first_name, $employee->last_name)]) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
